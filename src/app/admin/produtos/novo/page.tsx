...
import { productService, storageService } from '@/services/firebase-services';
...
export default function NovoProdutoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let imageUrl = "";
      if (formData.image) {
        imageUrl = await storageService.uploadProductImage(formData.image, Date.now().toString());
      }
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex items-center gap-4">
                <label className="cursor-pointer bg-gray-100 hover:bg-gray-200 px-4 py-2 rounded-lg flex items-center gap-2">
                  <Upload size={20} />
                  {formData.image ? 'Trocar Imagem' : 'Selecionar Imagem'}
                  <input
                    type="file"
                    accept="image/*"
